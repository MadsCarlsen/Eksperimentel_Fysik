...
def load_angles(angle_file):
    ...
    phi3 = phi1 - angle_list[2] #Vinkel af reflektion - burde være lig phi1
    return np.array([np.deg2rad(phi1),np.deg2rad(phi2),np.deg2rad(phi3),phi1])


def find_n(phi1,phi2):
    ...
    print(chi2.cdf(ch_min,len(sin2)-2))
    print(p_err[0]/p_opt[0]**2)
    print(1.003/p_opt[0])


def load_intensities(file_type,angles,int0): #OBS! Angles must be deg! Use load_angles[3].
    mean_list = []
    error_list = []
    for angle in angles:
        file = file_type + f'{int(angle)}.csv'
...
def theta2(theta1):
    return np.arcsin(ratio_air_glass*np.sin(theta1))
...
def Tp_model(theta1):
    return np.sin(2*theta1)*np.sin(2*theta2(theta1))/\
            ((np.sin(theta1 + theta2(theta1)))**2*(np.cos(theta1 - theta2(theta1))**2))

def Ts_model(theta1):
    return np.sin(2*theta1)*np.sin(2*theta2(theta1))/(np.sin(theta1 + theta2(theta1)))**2
